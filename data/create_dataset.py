...
lr_train_dir = "./train/input"  # Low resolution image at 2x.
hr_train_dir = "./train/target"  # High resolution image at 2x.
# Define the directory address of the dataset
lr_val_dir = "./val/input"
hr_val_dir = "./val/target"
...
# Delete old folder
if os.path.exists(lr_val_dir):
    shutil.rmtree(lr_val_dir)
if os.path.exists(hr_val_dir):
    shutil.rmtree(hr_val_dir)
# Create new folder
os.makedirs(lr_val_dir)
os.makedirs(hr_val_dir)
...
def split_dataset(lr_train_dir: str = None, hr_train_dir: str = None,
    ...
    for _, _, files in os.walk(lr_train_dir):
        ...
        for filename in samples:
            ...
            shutil.copyfile(os.path.join(lr_train_dir, filename),
                            os.path.join(lr_val_dir, filename))
            shutil.copyfile(os.path.join(hr_train_dir, filename),
                            os.path.join(hr_val_dir, filename))


if __name__ == "__main__":
    split_dataset(lr_train_dir=lr_train_dir, hr_train_dir=hr_train_dir,
                  lr_val_dir=lr_val_dir, hr_val_dir=hr_val_dir)
